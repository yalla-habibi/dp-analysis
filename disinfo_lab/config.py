...
class Config:
    # gdzie trzymamy artefakty (CSV, sqlite) — pod repo / w chmurze
    data_dir: str = _env("DISINFO_DATA_DIR", "data").strip()
    ...
    # --- DB ---
    # domyślnie do folderu data/ żeby łatwo było mirrorować w CSV
    db_url: str = _env("DISINFO_DB_URL", "sqlite:///data/disinfo_lab.sqlite3")

    # --- Ollama ---
    ollama_base_url: str = _env("OLLAMA_BASE_URL", "http://localhost:11434").strip()
    # domyślnie taki, który zwykle istnieje (u Ciebie działa)
    ollama_model: str = _env("OLLAMA_MODEL", "llama3.1:latest").strip()
